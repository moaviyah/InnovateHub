...
const Docs = ({ navigation }) => {
    ...
    const linksQuery = query(resourcesRef, orderByChild('category'), equalTo('Media'));
    const auth = getAuth()
    useEffect(() => {
        const fetchLinks = () => {
    ...
    const renderItem = ({ item, index }) => (
        <TouchableOpacity style={styles.linkContainer}>
            <Image source={{uri: item.media[0].uri}} style={{height:250, width:320, alignSelf:'center', borderRadius:10}}/>
